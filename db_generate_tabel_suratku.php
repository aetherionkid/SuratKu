...
try {
  ...
  // Buat database "Suratku" (jika belum ada)
  $query = "CREATE DATABASE IF NOT EXISTS suratku";
  $mysqli->query($query);
  if ($mysqli->error){
    throw new Exception($mysqli->error, $mysqli->errno);
  }
  else {
    echo "Database 'suratku' berhasil di buat / sudah tersedia <br>";
  };

  // Pilih database "suratku"
  $mysqli->select_db("suratku");
  if ($mysqli->error){
    throw new Exception($mysqli->error, $mysqli->errno);
  }
  else {
    echo "Database 'suratku' berhasil di pilih <br>";
  };
